...
### UI 任务必须使用 `ui-ux-pro-max` 系列 Skill

- 当前会话的可用 Skill 中只要存在 `ui-ux-pro-max:*`，凡涉及用户可见界面的新增、改版或审查，包括页面、组件、布局、样式、配色、排版、响应式、动效、数据可视化和视觉资产，都必须在设计或修改 UI 代码前调用该系列 Skill；不能仅凭 Agent 自身默认审美直接实现。
- UI 页面或组件任务默认先调用 `ui-ux-pro-max:ui-ux-pro-max`，用它确定适合产品和场景的设计方向、信息层级、布局、颜色、字体、交互、响应式与可访问性要求；进入 Tailwind、shadcn/ui 或具体组件代码实现时，再调用 `ui-ux-pro-max:ui-styling`。
- 按任务补充调用最匹配的专项 Skill，不要求无差别调用整个系列：设计令牌、主题和组件规范使用 `ui-ux-pro-max:design-system`；品牌定位、语调和一致性检查使用 `ui-ux-pro-max:brand`；Logo、图标及综合视觉资产使用 `ui-ux-pro-max:design`；官网 Hero、活动横幅和广告图使用 `ui-ux-pro-max:banner-design`；HTML 演示或汇报页面使用 `ui-ux-pro-max:slides`。
- 调用 Skill 时必须提供足够上下文：目标用户、页面目标、核心操作、内容层级、已有界面或问题、技术栈、必须复用的组件与语义令牌，以及web响应式、亮/暗主题和可访问性约束。改版任务还要明确保留现有业务功能和交互契约，避免 Skill 为追求视觉效果擅自改变需求。
- Skill 的输出是设计与实现约束，不替代项目事实。落地时仍必须遵守本文件的架构、组件复用、品牌文案、隐私和验证规则；不得为了套用推荐风格而引入无关依赖、重写基础组件或扩大任务范围。
