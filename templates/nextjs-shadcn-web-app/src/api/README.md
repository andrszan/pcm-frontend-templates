...
这不是功能缺失，而是当前项目的扩展边界。确认后端契约后，应在现有结构上增加对应策略，而不是修改每个 endpoint。
...
项目不预建空的 `shared/`。只有同一个稳定后端契约被两个及以上模块使用时才创建：
...
项目不读取 Web Storage。浏览器项目可以注册请求拦截器：
