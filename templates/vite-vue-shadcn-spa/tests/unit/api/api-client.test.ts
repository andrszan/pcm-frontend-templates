...
describe('ApiClient', () => {
  it('默认返回响应数据而不是 AxiosResponse', async () => {
    const client = new ApiClient({ adapter: async (config) => response(config, { status: 'ok' }) })

    await expect(client.get<{ status: string }>('/status')).resolves.toEqual({ status: 'ok' })
  })

  it('保留配置请求体，并让显式请求体优先', async () => {
    const client = new ApiClient({ adapter: async (config) => response(config, config.data) })

    await expect(
      client.post<string, string>('/from-config', undefined, { data: 'configured' }),
    ).resolves.toBe('configured')
    await expect(
      client.post<string, string>('/explicit', 'explicit', { data: 'configured' }),
  ...
  it('可以单独卸载响应拦截器', async () => {
    const client = new ApiClient({ adapter: async (config) => response(config, 'original') })
    const eject = client.use(
  ...
  it('只对 Axios pipeline 错误执行一次错误标准化', async () => {
    ...
    expect(errorNormalizer).toHaveBeenCalledExactlyOnceWith(originalError)
  })
